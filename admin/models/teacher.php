 ...
 * Teacher model.
 */
class CooltouramanModelTeacher extends JModelAdmin
{
	 ...
	 *
	 */
	public function getTable($type = 'Teacher', $prefix = 'CooltouramanTable', $config = array())
	{
		...
		{  
		   ...
		   // Convert the skills field to an array.
			...
			$query = $db->getQuery(true)
				->select(array('a.*', 'b.title, b.duration_in_hours, b.start_date, b.end_date, b.attendanceregister'))
				->from($db->quoteName('#__cooltouraman_course_teacher_relations', 'a'))
				->join('LEFT', $db->quoteName('#__cooltouraman_course', 'b') . ' ON (' . $db->quoteName('b.id') . ' = ' . $db->quoteName('a.course_id') . ')')
				->where('a.teacher_id='.$item->id);
			...
			foreach($courses as $index => $course)
			{
				if (!empty($course->attendanceregister))
				{
					...
					$item->courses[$index]["end_date"] = $course->end_date;
					$item->courses[$index]["title"] = $course->title;
					$attendances = array_map(function ($ar) {return $ar['attendance'];}, $attendanceregister["teacher"]["attendances"]);
					$item->courses[$index]["request_attendances"] = count($attendances);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cooltouraman.teacher', 'teacher', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_cooltouraman.edit.teacher.data', array());
