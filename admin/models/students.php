 ...
 * @since  1.6
 */
class CooltouramanModelStudents extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'parental_firstname', 'a.parental_firstname',
				'parental_lastname', 'a.parental_lastname',
				'street', 'a.street',
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				'list.select',
				'a.*, COUNT(b.course_id) AS attended_courses_number'
			)
		);

		$query->from($db->quoteName('#__cooltouraman_student') . ' AS a');
		
			// Join over the assign courses
		$query->join('LEFT', $db->quoteName('#__cooltouraman_course_student_relations') . ' AS b ON b.student_id = a.id');
		...
		if ($this->getState('filter.search') !== '' && $this->getState('filter.search') !== null)
		{
			...
			$searches[] = 'a.parental_firstname LIKE ' . $search;
			$searches[] = 'a.parental_lastname LIKE ' . $search;
