...
$elemid     = $input->getCmd('elemid');
$function  = 'jSelectTeacher_' . $field;

$listOrder = $this->escape($this->state->get('list.ordering'));
$listDirn  = $this->escape($this->state->get('list.direction'));
?>
<form action="<?php echo JRoute::_('index.php?option=com_cooltouraman&view=teachers&layout=modal&tmpl=component'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<?php else : ?>
		<table class="table table-striped table-condensed">
			<thead>
				<tr>
					<th class="left">
						<?php echo JHtml::_('grid.sort', 'COM_COOLTOURAMAN_TEACHER', 'a.lastname', $listDirn, $listOrder); ?>
					</th>
					<th class="nowrap" width="25%">
						<?php echo JHtml::_('grid.sort', 'COM_COOLTOURAMAN_PRIMARY_PHONE', 'a.primary_phone', $listDirn, $listOrder); ?>
					</th>
					<th class="nowrap" width="25%">
			...
			<tbody>
				...
				foreach ($this->items as $item) : ?>
					<tr class="row<?php echo $i % 2; ?>">
						<?php $teacher = $this->escape($item->firstname . ' ' .$item->lastname); ?>
						<td>
							<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $item->id; ?>', '<?php echo $teacher; ?>', '<?php echo $elemid; ?>');">
								<?php echo $teacher;?>
							</a>
						</td>
						<td align="center">
							<?php echo JHtml::_('date', $item->primary_phone, 'd-m-Y'); ?>
						</td>
						<td align="left">
